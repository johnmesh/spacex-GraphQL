...
import LastLaunchView from './LastLaunchView'
import { DataProvider } from 'hooks/useDataProvider'
import { LastLaunchDataDocument } from 'hooks/useDataProvider'
import { launchLatestRes } from '../mock-data/lastRocketLaunch'

const mocks = [
  {
    request: {
      query: LastLaunchDataDocument,
    },
    result: {
      data: {
        launchLatest: { ...launchLatestRes },
      },
    },
  },
]

describe('LastLaunchView', () => {
  beforeEach(async () => {
    await act(async () => {
      render(
  ...
  it('should display the general details', async () => {
    ...
    expect(staticeDateEl.textContent).toEqual(
      launchLatestRes.static_fire_date_utc
    )
    ...
    expect(tentativeMaxPrecisionEl.textContent).toEqual(
      launchLatestRes.tentative_max_precision
    )
    ...
    expect(launchSuccessEl.textContent).toEqual(
      `${launchLatestRes.launch_success}`
    )
  ...
  it('should display the rocket details', async () => {
    const rocket = launchLatestRes.rocket.rocket
  ...
  it('Should display launch site details', async () => {
    const siteDetails = launchLatestRes.launch_site
  ...
  it('should display the links', async () => {
    const links = launchLatestRes.links
